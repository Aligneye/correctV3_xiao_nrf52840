  ...
  switch (action) {
    case ACTION_SINGLE_CLICK:
      ...
      if (currentState == POWER_OFF) {
        ...
        wakePostureSensor(); // Bring LIS3DH out of power-down
        initBLE();
        resetAutoOffTimer(); // Start the inactivity countdown fresh
        // RTC2 kept running through sleep, so time is still accurate.
        char stampBuf[24];
        formatEpochUTC(getDeviceTime(), stampBuf, sizeof(stampBuf));
        Serial.printf("--- WAKING FROM IDLE (time=%s UTC) ---\n", stampBuf);
        setTrackingMode();
      } else {
